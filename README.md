...
- 通过多轮对话收敛绘图需求
- 用户要求整理需求时，由对话模型直接输出 request 文本
- 用户显式确认开始绘图后，由对话模型调用单一建任务工具进入执行链路
- 使用 SQLite 持久化用户、会话、消息和任务
- 使用本地文件系统持久化生成图片
- 支持任务停止、结果重试和重启恢复
- 支持 `/new` 新建当前会话并切换后续对话上下文
- 支持 `/img` 调整默认尺寸与画师串，支持 `/balance` 查询 NAI 余额

## 交互流程

1. 用户发送普通文本，进入会话式需求收敛流程。
2. 当用户要求整理需求时，Bot 直接返回一条 request 文本，供用户人工确认。
3. 用户显式表达“开始绘图”后，对话模型调用 `create_drawing_task`，系统创建任务并进入异步执行。
...
6. 如果用户希望重新开始一轮需求收敛，可发送 `/new` 新建当前会话；旧会话与旧任务会保留用于后续审计。
...
- `conversation.recent_message_limit`: 每次生成会话回复或触发建任务时读取的最近消息条数，默认 `15`。
- `recovery.enabled`: 是否在启动时自动恢复 `queued / translating / drawing` 任务，默认开启。
- `llms`: OpenAI 兼容模型列表；`llms[0]` 用于会话回复、request 整理与建任务工具调用，整组 `llms` 用于 prompt 翻译 failover，请至少配置一个可用模型。
